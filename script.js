...
// Variabel tambahan untuk fitur kamera WebRTC
...
function renderAll(){
  ...
  // Jika kamera sedang dibuka, pasang aliran video (stream) ke tag HTML <video>
  if(mode==='kiosk' && kioskStep===2 && isCameraOpen && cameraStream){
    ...
    <div class="card">
      ...
      ` : kioskStep===2 ? `
        ...
        <div class="photo-zone">
          ${isCameraOpen ? `
            <video id="cameraStream" autoplay playsinline class="photo-preview" style="background:#000; transform: scaleX(-1);"></video>
            <div class="photo-actions">
              <button type="button" class="btn btn-primary btn-sm" id="btnSnap">📸 Jepret Foto</button>
              <button type="button" class="btn btn-outline btn-sm" id="btnCloseCamera">Batal</button>
            </div>
          ` : draftPhoto ? `
            <img src="${draftPhoto}" class="photo-preview">
            <div class="photo-actions">
              <button type="button" class="btn btn-primary btn-sm" id="btnOpenCamera">📷 Ambil Ulang</button>
              <button type="button" class="btn btn-outline btn-sm" data-trigger="uploadInput">⬆ Unggah Baru</button>
              <input type="file" id="uploadInput" accept="image/*" class="visually-hidden">
            </div>
          ` : `
            <div class="photo-placeholder">👤</div>
            <div class="photo-actions">
              <button type="button" class="btn btn-primary btn-sm" id="btnOpenCamera">📷 Buka Kamera</button>
...
function startCamera() {
  if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
    ...
    .then(function(stream) {
      cameraStream = stream;
      isCameraOpen = true;
      renderAll(); // Re-render untuk memunculkan tag video HTML
    })
    .catch(function(err) {
      showPhotoErr("Kamera diblokir atau tidak dapat diakses.");
    });
  } else {
    showPhotoErr("Browser ini tidak mendukung fitur kamera langsung.");
  }
}
...
function takeSnapshot() {
  ...
  // Karena video kita di-mirror (scaleX(-1)), hasil jepretan kanvas juga harus di-mirror agar tidak terbalik
  ctx.translate(canvas.width, 0);
  ctx.scale(-1, 1);
  ctx.drawImage(video, 0, 0, canvas.width, canvas.height);
  
  // Simpan foto dalam format Base64 JPEG
...
function showPhotoErr(msg){
  const el = document.getElementById('photoErr');
...
function wireEvents(){
  ...
  // Matikan kamera kalau menekan tombol kembali ke form
  if(backStep1) backStep1.addEventListener('click', ()=>{ stopCamera(); kioskStep=1; renderAll(); });
  ...
  if(btnCloseCam) btnCloseCam.addEventListener('click', ()=>{ stopCamera(); renderAll(); });
  
  const btnSnap = document.getElementById('btnSnap');
  if(btnSnap) btnSnap.addEventListener('click', takeSnapshot);

  // Fungsi upload manual tetap dipertahankan sebagai alternatif
  ...
  if(toStep3) toStep3.addEventListener('click', ()=>{
    // Jika foto belum ada, wajibkan ambil/unggah terlebih dahulu
    if(!draftPhoto) {
      showPhotoErr('Mohon ambil foto / selfie terlebih dahulu.');
      return;
    }
    stopCamera();
